...
```{r}
# Scale and subset
sce <- scale_sub(sce, assay_name = "counts_gc_modal", scale = "cell", log2 = TRUE)

# Calculate CNV score
sce <- calc_cnv_score(sce, assay_name = "counts_gc_modal_scaled_cell")

# Convert to seurat object
srt <- as.Seurat(sce, counts = "counts", data = "counts_gc_modal_scaled_cell")

# Scale (but don't actually scale as it's done already)
srt <- ScaleData(srt, do.scale = FALSE, do.center = FALSE, verbose = FALSE)

# Run PCA, and clustering workflow
srt <- RunPCA(srt, features = rownames(srt), verbose = FALSE)
srt <- FindNeighbors(srt, dims = 1:50, verbose = FALSE)
srt <- FindClusters(srt, resolution = c(0.5), algorithm = 1, verbose = FALSE)
srt <- RunUMAP(srt, dims = 1:50, n.neighbors = 10, metric = "correlation", verbose = FALSE)

# Put the PCA, UMAP, and clustering results into the original SCE
reducedDim(sce, "PCA") <- srt@reductions$pca@cell.embeddings
reducedDim(sce, "UMAP") <- srt@reductions$umap@cell.embeddings
sce$seurat_clusters <- srt[[]]$seurat_clusters
```

Plot the results
```{r}
# Color palette
col_pal <- scales::hue_pal()(length(levels(srt[[]]$seurat_clusters)))
names(col_pal) <- levels(srt[[]]$seurat_clusters)

p1 <- DimPlot(srt, group.by = "seurat_clusters", reduction = "umap", label = TRUE) + scale_color_manual(values = col_pal) + NoLegend()
p2 <- FeaturePlot(srt, features = "cnv_score", label = TRUE) + scale_color_viridis_c()
# p2 <- DimPlot(srt, group.by = "tumor_type", reduction = "umap", label = FALSE) + scale_color_colorblind()
...
ht <- cnaHeatmap(sce, assay_name = "counts_gc_modal_scaled_cell", clone_name = "seurat_clusters", legend_name = "cell scaled", col_clones = col_pal, border = TRUE)
ht
```

A clone summarized heatmap
```{r cnaheatmap_plot2, message=FALSE}
ht2 <- cloneCnaHeatmap(sce, assay_name = "counts_gc_modal_scaled_cell", clone_name = "seurat_clusters", legend_name = "clone avg", col_clones = col_pal)$plot
ht2
```
...
```{r cell_cna_plot}
plot_cell_cna(sce = sce, cell_id = colnames(sce)[1:5], assay_name = "counts_gc_modal_scaled_cell")
```
